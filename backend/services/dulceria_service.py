...
def get_catalogo(complejo_slug: str, db: Session):
    consulta = text("""
        SELECT 
            ...
            mr.regla_id, mr.titulo_regla, mr.min_items, mr.max_items, mo.opcion_id, mo.nombre_opcion, mo.precio_adicional
        FROM COMPLEJO c
    ...
    for row in resultados:
        ...
        if regla_id:
            ...
            if row.opcion_id:
                prod_obj[regla_id]["opciones"].append({"opcion_id": row.opcion_id, "nombre": row.nombre_opcion, "precio_extra": float(row.precio_adicional)})
